local d6_side_info = SMODS.D6_Side({
	...
	loc_txt = {},
	config = {mult = 80, odds = 2, mult_minus = 30},
	pos = {x=3, y=4},
	loc_vars = function(self, info_queue, card, d6_side)
		return {vars = {''..(G.GAME and G.GAME.probabilities.normal or 1), d6_side.extra.odds, d6_side.extra.mult, d6_side.extra.mult_minus}}
	end,
	calculate = function(self, card, context, d6_side)
		if context.joker_main then
			...
			if pseudorandom("all_in") > G.GAME.probabilities.normal/d6_side.extra.odds then
				...
				return {
					message = localize{type='variable',key='a_mult_minus',vars={d6_side.extra.mult_minus}},
					mult_mod = -d6_side.extra.mult_minus,
					colour = G.C.MULT
				}, true
			end
		end
...
D6_JokerDisplay.D6_Side_Definitions[d6_side_info.key] = {
	text = {
		...
		{ text = ' -' },
		{ ref_table = 'card.joker_display_values', ref_value = 'mult_minus' },
	},
